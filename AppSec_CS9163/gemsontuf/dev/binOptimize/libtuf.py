...
class Repository(object):
  ...
  def write(self, write_partial=False):
    ...
    class threadWrite(threading.Thread):
      def __init__(self, repoDir, tarDir, delRole, roleInfo, writePart):
        threading.Thread.__init__(self)

        self.repoDir = repoDir
        self.tarDir = tarDir
        self.delRole = delRole
        self.roleInfo = roleInfo
        self.writePart = writePart

      def run(self):
        write_delegated_metadata_file(self.repoDir,
                                      self.tarDir,
                                      self.delRole, self.roleInfo, self.writePart)

    class controlQueue(threading.Thread):
    ...
    for delegated_role in delegated_roles:
      ...
      k = 0
      try:
        k = roleinfo['paths'][0][0:4]
      except ValueError, e:
        delUnDir = self._repository_directory
        delUnTar = self._targets_directory
        delRole = delegated_role
        delInfo = roleinfo
        continue
      except IndexError:
        continue
      ...
      tmp = threadWrite(self._repository_directory,
    ...
    a = controlQueue("first", fir)
    b = controlQueue("second", sec)
    c = controlQueue("third", thr)
    d = controlQueue("forth", fur)

    print("Start delegate write")
    a.start()
    b.start()
    c.start()
    d.start()
    ...
    write_delegated_metadata_file(delUnDir, delUnTar, delRole, delInfo, write_partial)
