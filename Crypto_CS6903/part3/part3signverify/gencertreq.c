...
void usage ( void ) {
    fprintf ( stderr, "\n\nUsage: ./gencertreq  publickey.txt crs.txt\n");
    fprintf ( stderr, "where:\n" );
    fprintf ( stderr, "       publickey.txt -  public key of the certificate requestor ");
    fprintf ( stderr, "       crs.txt - binary file containing certificate signing request from a public key owner\n");
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "\n" );
}
...
// builds the file containing parameters to use for certificate generation request
int main( int argc, char * argv[] ) {
    ...
    if ( argc != 3 ) {
        /* must have public key and binary output file specified */
        usage();
        exit (1);
    }
    ...
    /* Write the file */
    if(!PEM_write_RSAPublicKey(out_file, rsa_pub)){
