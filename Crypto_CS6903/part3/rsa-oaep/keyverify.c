...
void usage ( void ) {
    fprintf ( stderr, "\n\nUsage: ./keyverify publickey.txt certificate.txt validityparameters.txt\n");
...
//    pub_key = PEM_read_PUBKEY( public, NULL, NULL, NULL );
    ...
    /* check the certificate signature using key in the certificate */
    result = X509_verify( cert, cert_pub_key );
...
//        perror( "Error printed by perror" );
    ...
    /* compare public key with public key in certificate */
    result = EVP_PKEY_cmp ( pub_key, cert_pub_key );
    ...
    if ( result != 1  ) {
//        fprintf( stderr, "No Match On Public Key And Public Key Contained In Certificate\n");
        fprintf( stdout, "NO\n");
        fflush ( stdout );
        exit(1);
    }
